...
def main(args):
    ...
    # 修正剪枝后模型的配置信息
    # 关键：只修改每层的 self_attn 属性，不修改全局 config
    # 因为 PEFT/LoRA 在初始化时会读取 config，修改 config 会导致维度不匹配
    print("\n" + "=" * 80)
    print("检查并修正模型配置...")
    print("=" * 80)
    ...
    print(f"全局 config (保持不变，确保 LoRA 正确初始化):")
    print(f"  num_attention_heads: {model.config.num_attention_heads}")
    print(f"  num_key_value_heads: {model.config.num_key_value_heads}")
    print(f"  hidden_size: {model.config.hidden_size}")

    # 关键修复：逐层更新每个 Attention 层内部的 num_heads 和 num_key_value_heads
    # 这些值在 forward 时用于 reshape 和 repeat 操作
    print(f"\n逐层修正 Attention 配置（基于实际权重维度）:")
    print(f"{'Layer':<10} {'Q Heads':<12} {'KV Heads':<12} {'Ratio':<10}")
    print("-" * 80)
    ...
    for i, layer in enumerate(model.model.layers):
        ...
        print(f"Layer {i:<5} {layer_q:<12} {layer_kv:<12} {layer_q//layer_kv}:1")

    print(f"\n✅ 所有层的 Attention 配置已根据实际权重更新")
    print(f"⚠️  注意：全局 config 保持原始值，以确保 LoRA 正确初始化")
    print("=" * 80 + "\n")
